...
arima_cycle0 <- function(ts.train, ts.test, target, predictors){
  ...
  set.seed(123); to_return1 <- ts.train %>%
...
env_meta <- env_meta[match(unique(env_meta$DateTime), env_meta$DateTime),]
...
```{r Multimodel forecasting, cache=TRUE}
...
for(i in 1:17){
  ...
  to.plot.cycle <- combined %>%
    ...
    filter(.model%in%(a1 %>% arrange(RMSE) %>% pull(.model))[1]) %>%
  ...
  values.cycle <- ts.tot %>%
    mutate(real.value=value, data.set=ifelse(as.Date(date) < as.Date("2012-05-28"), "train", "test")) %>% # Changed from "2012-06-01" to "2012-05-28"
...
signal.forecast.plot <- best.models %>%
  ...
  geom_ribbon(aes(ymin=q05, ymax=q95, fill=.model), alpha=0.3) +
  geom_line(aes(group=.model, color=.model)) +
  geom_point(data=subset((best.models %>%
                            mutate(data.set=case_when(date>as.Date("2012-05-01") ~ "test",
                            .default = "train"))),
                         data.set=="train"), aes(x=as.Date(date), y=real.value), color="darkgreen") +
  geom_point(data=subset((best.models %>%
                            mutate(data.set=case_when(date>as.Date("2012-05-01") ~ "test",
                            .default = "train"))), data.set=="test"), aes(x=as.Date(date), y=real.value), color="darkblue") +
  theme_classic() +
  facet_wrap(factor(signal, levels=paste0("S", 1:17))~., ncol=2)

rmse.plot <- best.models %>%
  mutate(date=as.Date(date),
         Set=case_when(date < as.Date("2012-05-28") ~ "Train", # beginning of W22 2012
                       date>="2012-05-01" & date<"2013-06-01" ~ "Test2012",
                       date>="2013-06-01" & date<"2014-06-01" ~ "Test2013",
                       date>="2014-06-01" & date<"2015-06-01" ~ "Test2014",
                       date>="2015-06-01" & date<"2016-06-01" ~ "Test2015",
                       date>="2016-06-01" & date<"2017-06-01" ~ "Test2016")) %>%
...
WN.P.table <- best.models %>% mutate(date=as.Date(date),
                     Set=case_when(date < as.Date("2012-05-28") ~ "Train",
                       date>="2012-05-01" & date<"2013-06-01" ~ "Test2012",
                       date>="2013-06-01" & date<"2014-06-01" ~ "Test2013",
                       date>="2014-06-01" & date<"2015-06-01" ~ "Test2014",
                       date>="2015-06-01" & date<"2016-06-01" ~ "Test2015",
...
WN.P.table
WN.P.parsed <- WN.P.table %>%
  mutate(Train_significative=if_else(Train<0.01, 1, 0),
         Test_significative=if_else(Test<0.01, 1, 0)) %>%
  separate(signal, into = c("S", "num"), 1, remove=F) %>%
  mutate(num=as.numeric(num)) %>%
  arrange(num) %>%
  dplyr::select(signal, Train, Test, Train_significative, Test_significative)
WN.P.parsed

write.csv(WN.P.parsed, file=paste0(wd, "/results/tables/residuals_Ps.csv"), row.names=F)
...
```{r Output EVs}
signals.EVs %>%
  filter(module %in% c("S1", "S4", "S7", "S9", "S14", "S17")) %>%
  pull(EV) %>%
  sum()

write.csv(signals.EVs, file=paste0(wd, "/results/tables/signals_EVs.csv"), row.names=F)
```
...
June.average.dd <- June.dd %>%
  group_by(Year, block, variable, omic, block_omic) %>%
  summarise(M.pred = mean(pred.value), M.real = mean(real.value))


write.csv(June.average.dd, file=paste0(wd, "/results/tables/June_average.csv"), row.names=F)

write.csv(June.Rsq.table, file=paste0(wd, "/results/tables/June_Rsq.csv"), row.names=F)

to_print_models <- as.data.frame(best.models) %>%
  dplyr::select(.model, date, value, real.value)

write.csv(to_print_models, file=paste0(wd, "/results/tables/best_models.csv"), row.names=F)

to_plot_arima <- Omics.expl.Ps %>%
  mutate(expl.block=expl_block[expl.var], expl.short=expl_short[expl.var]) %>%
  mutate(EG_name=ifelse(mat_name=="raw",
                        paste0(Omic, ".", mat_name, ".a.EG", EG_num),
                        paste0(Omic, ".", mat_name, ".EG", EG_num))) %>%
  ...
  filter(EG_num!=0, !is.nan(Pvalue)) %>%
  mutate(Pvalue=p.adjust(Pvalue, method="BH")) %>%
  mutate(signal_name=factor(signal_name,
                            levels=rev(levels(signal_name))),
         sign.value=factor(sign(est.value)),
         est.value=abs(est.value)) %>%
  dplyr::select(Omic, expl.var, Pvalue, est.value, expl.block, expl.short, signal_name, sign.value)

write.csv(to_plot_arima, file=paste0(wd, "/results/tables/arima_explained.csv"), row.names=F)
...
save.image(file=paste0(wd, "/preprocessed/output.RData"))
